...
'''try:
    cur.execute("Create table book(bid varchar(20), title varchar(30), author varchar(30), status varchar(30))")
...
def bookRegister():
    ...
    status = status.lower()
    insertBooks = "insert into book values ('"+bid+"','"+title+"','"+author+"','"+status+"')"
    try:
...
def addBook(): 
    ...
    drop = OptionMenu( root , bookInfo4 , *options )
    drop.pack()
    drop.place(relx=0.4,rely=0.65, relwidth=0.2, relheight=0.04)
    '''
